...
def get_Image(item, count):
    try:
        ...
        with open("Perfumes" + "/" + str(count) + '.png', 'wb') as handler:
            handler.write(image)
    except AttributeError:
        image = ""
# ====================================== Product Type ======================================
...
if __name__ == '__main__':
    ...
    URL = "https://www.junaidjamshed.com/fragrances/for-men.html?p=1"
    ...
    for item in items:
    path = os.getcwd()
    path = os.path.join(path, "Perfumes")
    os.mkdir(path)
    with open("Perfumes" + "/" + "Perfumes" + ".csv", "w") as outputfile:
